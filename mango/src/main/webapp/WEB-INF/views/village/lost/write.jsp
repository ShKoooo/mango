...
/*
$(function() {
	var img = "${dto.vimgFilename}";
	
	if (img) { //수정일때
		img = "${pageContext.request.contextPath}/uploads/lost/"+img;
		$(".write-form .img-viewer").empty();
		$(".write-form .img-viewer").css("background-image", "url("+img+")");
	}
	
	$(".write-form .img-viewer").click(function() {
		$("form[name=lostForm] input[name=selectFile]").trigger("click");
	});
	
	$("form[name=lostForm] input[name=selectFile]").change(function() {
